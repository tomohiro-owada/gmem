...
この候補は Hugging Face 上で ONNX タグがあり、license は MIT と表示されている。bge 系モデルは BAAI の Hugging Face organization で配布されている。
...
2. Git pull で記憶リポジトリを最新化する
3. `title` と `content` に対して security gate を実行する
4. 拒否対象が見つかった場合は Markdown、SQLite、Git、ログのいずれにも本文を残さずに失敗を返す
5. `projects/{project_id}/` 配下にユニークな Markdown ファイルを生成する
6. embedding を生成する
7. embedding に失敗した場合でも Markdown 保存は続行し、SQLite には `embedding_status=pending` として登録できる状態にする
8. Git add / commit を実行する
9. Git push を実行する
10. push 成功後、SQLite に登録する
...
5. retry push が失敗した場合は、pull と検索を実行せず、未 push commit が残っていることをエラーとして返す
6. Markdown と SQLite の差分を再インデックスする
7. `query` の embedding を生成する
8. `project_id` 指定があれば絞り込む
9. ベクトル類似度で上位結果を返す
...
検索結果はデフォルトで全文を返しすぎない。AI agent 向けには `--fields` と `--snippet-chars` により、必要な情報だけを返せるようにする。
...
- pending / failed embedding count
...
Go MCP Server
  ...
  | generate embedding or mark embedding_status=pending
  | git add / commit
  | git push
  | update sqlite index
  v
...
- embedding 失敗時でも Markdown 保存は成功させ、SQLite では `embedding_status=pending` として後続の `sync` で再処理する
...
- embedding なしで Markdown 保存だけは成功させ、`embedding_status=pending` として扱う
- 未 embedding のファイルを後から再処理する
...
- 将来的に allowlist や明示的な手動 override を検討する
...
- security gate の手動 override を許可するか
- `search` の返却本文は全文、抜粋、指定文字数のどれをデフォルトにするか
- bge 系軽量 ONNX model の具体的な配布元、checksum、ライセンスをどれに固定するか
- search 時に retry push が失敗した場合、ローカル状態で検索を続行するか、エラーとして止めるか
