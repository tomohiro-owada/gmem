...
AIエージェントを毎回ゼロから教え直している、という経験がある。

「このプロジェクトではこの方針でいく」「このAPIはこういう理由で使わない」「前にこのバグでハマった」——そういう話を、会話を跨いで持ち越せない。毎回コンテキストを貼り直している。

ならどこかに保存すればいい。でも、ベクトルDBに突っ込むのはちょっと怖い。人間が読めない記憶は、あとから確認できない。「なぜこの判断が保存されたのか」が追えない。

なので自分で作りました。

https://github.com/tomohiro-owada/gmem

名前は `git-mcp-memory` です。AIの記憶をGit管理されたMarkdownとして残しつつ、検索だけSQLiteとローカルembeddingで速くするMCP server / CLIです。

![AIに無料で手軽に長期記憶をもたせるツールを作りました](/images/ai-agent-memory-git-mcp/problem-map.svg)

## なぜMarkdownとGitなのか

AIの記憶というと、まずベクトルDBを考えます。意味検索は強い。「あの話に近い記憶」を拾える。

でも、記憶の正本をベクトルDBに置くと、人間が確認しづらい。

「なぜこの判断が保存されたのか」「いつ保存されたのか」「前と何が変わったのか」——ここが見えなくなります。

逆にMarkdownだけにすると、今度は検索が弱い。ファイル名やキーワードを覚えていないと見つけにくい。
...
SQLiteが壊れても、Markdownから作り直せます。正本はGitに残る。ここが一番大事です。
...
全部をちゃんとしたSaaSにすると重い。GitHubのリポジトリとローカルのSQLiteで十分です。
...
長期記憶のために、毎回クラウドのembedding APIを呼ぶのは避けたかった。Ollamaのような外部API serverを常駐させるのも、依存が増えて面倒です。
...
AIエージェントは作業中にAPI keyやtokenを見ます。それをうっかり保存されたら困る。なので保存前にsecurity gateを通します。
